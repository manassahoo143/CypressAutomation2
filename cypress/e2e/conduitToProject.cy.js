...
describe("Conduit", ()=> {
    ...
    it("SignUp", ()=> {
        ...
        cy.get("input[placeholder='Article Title']").type("DemoTest@12396#!");
        cy.get("#main > div > div > div > div > div > form > fieldset > fieldset:nth-child(2) > input").type(randomUserId());
        cy.get("#main > div > div > div > div > div > form > fieldset > fieldset:nth-child(3) > textarea").type("Cav Test Case");
...
/*
    it("New post and verify", ()=> {
        cy.get("a[href='#editor']").click();
        cy.url().should('contain','editor');
        cy.get("input[placeholder='Article Title']").type("DemoTest@123!");
        cy.get("#main > div  fieldset > fieldset:nth-child(2) > input").type(randomUserId());
        cy.get("textarea[placeholder='Write your article (in markdown)']").type("dsnunwdciun");
        cy.get("button").contains('Publish Article').click();
